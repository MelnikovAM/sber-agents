...
def init_ragas_metrics():
    ...
    #langchain_embeddings = OpenAIEmbeddings(model=config.RAGAS_EMBEDDING_MODEL)
     ...
     # Настройка LLM для RAGAS (всегда OpenAI-совместимый) с retry настройками
    langchain_llm = ChatOpenAI(
        model=config.RAGAS_LLM_MODEL, 
        temperature=0,
        max_retries=10,  # Больше попыток при ошибках
        timeout=120,  # Увеличенный таймаут
        request_timeout=120  # Таймаут для запроса
    )
    ...
    if config.RAGAS_EMBEDDING_PROVIDER == "huggingface":
        ...
        langchain_embeddings = HuggingFaceEmbeddings(
            ...
            model_kwargs={
                'device': 'cpu',
                'trust_remote_code': True  # Разрешаем кастомный код из HuggingFace
            },
    ...
    # Настройки для выполнения (оптимизировано для rate limits)
    run_config = RunConfig(
        max_workers=1,  # Последовательная обработка
        max_wait=600,  # Увеличено время ожидания до 10 минут
        max_retries=10,  # Больше попыток при ошибках
        timeout=120  # Увеличенный таймаут для каждого запроса
    )
...
def evaluate_dataset(dataset_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Запускаем evaluation с retry logic для rate limits
    max_retries = 3
    retry_delay = 120  # секунд между попытками (увеличено)
