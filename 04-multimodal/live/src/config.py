...
class Config:
    ...
    # STT Configuration (отдельные настройки для Whisper)
    STT_PROVIDER = os.getenv("STT_PROVIDER", "local")  # "openai" (облачный API) или "local" (openai-whisper)
    STT_MODEL = os.getenv("STT_MODEL", "small")  # Для openai-whisper: "tiny", "base", "small", "medium", "large"
    STT_API_KEY = os.getenv("STT_API_KEY", os.getenv("OPENAI_API_KEY"))  # Отдельный ключ для STT или общий
    STT_BASE_URL = os.getenv("STT_BASE_URL", "https://api.openai.com/v1")  # URL для OpenAI Whisper API
