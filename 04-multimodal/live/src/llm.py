...
async def get_transaction_response_voice(
    ...
    """
    Обрабатывает транскрибированное голосовое сообщение и извлекает транзакции.
    Использует специальный промпт для голосовых сообщений, который учитывает
    особенности STT (опечатки, неточности распознавания).
    """
    try:
        logger.info(f"Processing transcribed voice message (length: {len(transcribed_text)}): {transcribed_text[:200]}...")
        ...
        raw_content = response.choices[0].message.content
        logger.info(f"Raw LLM response for voice (length: {len(raw_content) if raw_content else 0}): {raw_content[:1000] if raw_content else 'EMPTY'}")
        
        # Проверяем что ответ не пустой
        if not raw_content or not raw_content.strip():
            logger.error("LLM returned empty response for voice")
            raise ValueError("LLM returned empty response")
        ...
        try:
            ...
            parsed_response = TransactionResponse.model_validate(parsed_json)
            logger.info(f"Successfully parsed TransactionResponse for voice: transactions={len(parsed_response.transactions)}")
            return parsed_response
        except json.JSONDecodeError as json_error:
            logger.error(f"Failed to parse JSON from LLM response for voice: {json_error}")
            logger.error(f"Full response content ({len(raw_content)} chars): {raw_content}")
            raise
        except Exception as parse_error:
            logger.error(f"Failed to parse LLM response as TransactionResponse for voice: {parse_error}")
            logger.error(f"Full response content ({len(raw_content)} chars): {raw_content}")
            raise
    except (APIError, InternalServerError) as e:
        logger.error(f"LLM API error for voice: {e}")
        raise
    except Exception as e:
