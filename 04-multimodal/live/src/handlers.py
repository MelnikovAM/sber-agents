...
from llm import get_transaction_response_text, get_transaction_response_image, get_transaction_response_voice
...
async def handle_voice(message: Message):
    ...
    # Временный файл для голосового сообщения
    ...
    try:
        ...
        # Создаем временный файл с расширением .ogg (формат Telegram)
        with tempfile.NamedTemporaryFile(mode='wb', suffix='.ogg', delete=False) as temp_file:
            temp_audio_path = temp_file.name
            
            # Скачиваем файл
            file_buffer = await message.bot.download_file(voice_file.file_path)
            temp_file.write(file_buffer.getvalue())
        
        logger.info(f"Voice message downloaded to {temp_audio_path}, size: {os.path.getsize(temp_audio_path)} bytes")
        
        # Транскрибируем аудио
        transcribed_text = await transcribe_audio(temp_audio_path)
        
        if not transcribed_text or not transcribed_text.strip():
            logger.warning(f"Empty transcription result for {chat_id}")
            await message.answer(
                "Извините, не удалось распознать голосовое сообщение. "
                "Пожалуйста, попробуйте еще раз или отправьте текстом."
            )
            return
        
        logger.info(f"Transcription result for {chat_id}: {transcribed_text}")
        
        # Получаем историю сообщений без системного промпта для контекста
        message_history = chat_conversations[chat_id][1:] if chat_conversations[chat_id] else []
        
        # Получаем ответ LLM с structured output
        response = await get_transaction_response_voice(transcribed_text, message_history)
        ...
        if response.transactions:
            ...
            logger.warning(f"No transactions extracted from voice for {chat_id}")
        ...
        # Формируем ответ пользователю с транскрипцией
        answer_lines = [
            f"🎤 **Транскрипция:** _{transcribed_text}_",
            "",
            response.answer
        ]
        ...
        # Добавляем голосовое сообщение в историю как транскрибированный текст
        chat_conversations[chat_id].append(
        ...
        await message.answer(
            "Извините, произошла ошибка на стороне провайдера LLM или STT. "
            "Пожалуйста, попробуйте еще раз через несколько секунд."
        )
    except Exception as e:
        ...
        if temp_audio_path and os.path.exists(temp_audio_path):
            try:
                ...
                logger.warning(f"Failed to remove temporary file {temp_audio_path}: {e}")
