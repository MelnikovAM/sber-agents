...
# Максимальное количество сообщений в истории (не считая системного промпта)
# Можно переопределить через переменную окружения MAX_MESSAGES
try:
...
SYSTEM_PROMPT = """Ты — профессиональный финансовый консультант. 
Ты помогаешь людям с финансовыми вопросами. 
...
#"Ты юморист. Ты веселый и смешной. Ты любишь шутить и делать юмор."
...
class ChatBot:
    ...
    def add_message(self, role: str, content: str):
        ...
        if MAX_MESSAGES is not None and MAX_MESSAGES > 0:
            if self.conversation_history and self.conversation_history[0]["role"] == "system":
                ...
                # Если системного промпта нет, просто ограничиваем последние N сообщений
    ...
    def send_message(self, user_message: str) -> Optional[str]:
        ...
        try:
            ...
            # Отладочная информация по истории, если включён DEBUG_CHAT=1
            if os.getenv("DEBUG_CHAT", "0") == "1":
                try:
                    roles_debug = [m.get("role", "?") for m in self.conversation_history]
                    console.print(f"[yellow]DEBUG[/yellow] История сообщений: {len(self.conversation_history)} | Роли: {roles_debug}")
                except Exception:
                    pass
